...
const Sidebar = () => {
  ...
  const normalLink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md text-white dark:text-gray-200  hover:bg-blue-900	 m-2';
...
{/* className="h-[60px] bg-[#4FA1F6] w-[360px] md:overflow-hidden overflow-auto md:hover:overflow-auto pb-2 " */}    
          ...
          <div className="bg-[#4FA1F6]      
           w-[360px] md:overflow-hidden md:hover:overflow-auto pb-2 overflow-y-auto h-[60px] ..."  > 
          <Link to="/" onClick=
          {handleCloseSideBar} className="items-center gap-4 ml-7
          mt-3.5 flex text-xl font-extrabold tracking-tight 
           dark:text-white  text-white "  >
             <img  src={MyImage} type='js.png' /> <span className="text-2xl" >dojo</span>
          <div>
            <TooltipComponent content="Menu" position='BottomCenter'>
              <button type="button" onClick={() => setActiveMenu((prevActiveMeu) => 
              !prevActiveMeu)} 
        ...
        {links.map((item) => (
          <div key={item.title} >
            ...
            {item.links.map((link) => (
              ...
              <NavLink 
              to={`/${link.name}`}
              key={link.name} style={{color:'#FFFFFF', }}
            ...
            isActive ? activeLink : normalLink}
              > {link.image}
                ...
                <button  type="button" onClick={() => {}}  className="flex-end ml-12">
                {link.icon2}               
                </button>
        ...
        <border1 style={{border: '1px solid rgba(231, 234, 238, 0.2)', 
                          position:'absolute', 
                          width: '360px', 
                          height:'0px', 
                          left:'0px', 
